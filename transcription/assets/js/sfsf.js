...
Design goal is to have only a few heavily overloaded functions
*/
var init = function(_){
    var sfsf = {
    /**
    ...
    readEntriesWithMetadata: function(dirEntry, callback){
        var that = this;
        if(_.isString(dirEntry)) {
            that.cretrieve(dirEntry, {}, function(err, dirEntry){
                if(err){
                    callback(err);
                }
                that.readEntriesWithMetadata(dirEntry, callback);
            });
            return;
        }
    ...
    cretrieve : function(path, options, callback) {
        ...
        sfsf.politelyRequestFileSystem({}, function(error, fileSystem) {
            ...
            getDirectoryHelper(fileSystem.root);
        });
    }
    };
    return sfsf;
